...
class NetworkParser {
  public:
    ...
    /**
     * Read optional "fb_rows" value for FlattenedButterfly dimensions.
     * Returns 0 for non-FlattenedButterfly dimensions (auto-factorize from npus_count).
     *
     * @return fb_rows per each dimension (0 = auto)
     */
    [[nodiscard]] std::vector<int> get_fb_rows_per_dim() const noexcept;

    /**
     * Read optional "elec_bandwidth" value (electrical RDL bandwidth for
     * adjacent FlattenedButterfly tiles). When absent, defaults to the
     * per-dim "bandwidth" so the link stays uniform (legacy behaviour).
     *
     * @return electrical bandwidth per each dimension
     */
    [[nodiscard]] std::vector<Bandwidth> get_elec_bandwidths_per_dim() const noexcept;

    /**
     * Read optional "elec_latency" value (electrical RDL latency for
     * adjacent FlattenedButterfly tiles). When absent, defaults to the
     * per-dim "latency" so the link stays uniform (legacy behaviour).
     *
     * @return electrical latency per each dimension
     */
    [[nodiscard]] std::vector<Latency> get_elec_latencies_per_dim() const noexcept;
  ...
  private:
    ...
    /// optional row count for FlattenedButterfly dims (0 = auto-factorize)
    std::vector<int> fb_rows_per_dim;

    /// optional electrical RDL bandwidth per dim (defaults to bandwidth_per_dim)
    std::vector<Bandwidth> elec_bandwidth_per_dim;

    /// optional electrical RDL latency per dim (defaults to latency_per_dim)
    std::vector<Latency> elec_latency_per_dim;
