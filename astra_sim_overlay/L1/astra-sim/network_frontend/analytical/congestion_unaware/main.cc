...
int main(int argc, char* argv[]) {
    ...
    while (!exit) {
      ...
      // Track whether any NPU was awake-and-finished (i.e. needed servicing via
      // std::getline) during this pass. If a whole pass services nobody and the
      // event queue is drained, every system is asleep — all instances already
      // received "done" — and no further getline will ever occur to receive an
      // "exit". This happens in the degenerate EP case where only one instance
      // ran real work (e.g. N=1 over a 32-way EP group) and the others ran only
      // dummy waves: the controller's single "exit" lands while that one real
      // instance is also being put to sleep, so the loop would otherwise spin
      // forever printing check lines (deadlocking the Python controller, or
      // OOM-ing it). See LLMServingSim serving/core/controller.py::check_end.
      ...
      // All systems asleep and no events left: nothing will ever fire another
      // getline, so break out and fall through to the non-exited check (which
      // prints the proper "All Request Has Been Exited" handshake). Without
      // this the loop spins forever in the degenerate single-active-instance
      // EP case described above.
      if (!any_active && event_queue->finished()) {
        exit = true;
      }
    }
